...
class BasicBlock(ResNetBlockBase):
    def __init__(self, in_channels, out_channels, *, stride=1, norm="BN", k_size=3):
        """
        ...
        self.eca = eca_layer(in_channels*4, k_size)

        for layer in [self.conv1, self.conv2, self.shortcut, self.eca.conv]:
    ...
    def forward(self, x):
        ...
        out = F.relu_(out)
        out = self.conv2(out)
        # Add efficient channel attention network at here.
        out = self.eca(out)
...
# This is the function used to construct Resnest
class BottleneckBlock(ResNetBlockBase):
    def __init__(
        ...
        *,
        ...
        if self.radix>1:
            for layer in [self.conv1, self.conv3, self.shortcut, self.eca.conv]:
                if layer is not None:  # shortcut can be None
                    weight_init.c2_msra_fill(layer)
        else:
            for layer in [self.conv1, self.conv2, self.conv3, self.shortcut, self.eca.conv]:
                if layer is not None:  # shortcut can be None
                    weight_init.c2_msra_fill(layer)
...
class BasicStem(nn.Module):
    def __init__(self, in_channels=3, out_channels=64, norm="BN",
                 deep_stem=False, stem_width=32, k_size=3):
        """
        Args:
            norm (str or callable): a callable that takes the number of
                channels and return a `nn.Module`, or a pre-defined string
        ...
        if self.deep_stem:
            ...
            self.eca = eca_layer(in_channels*4, k_size)
            for layer in [self.conv1_1, self.conv1_2, self.conv1_3, self.eca.conv]:
    ...
    def out_channels(self):
        if self.deep_stem:
            return self.eca.conv.out_channels
        else:
...
def build_resnet_backbone(cfg, input_shape):
    ...
    # This is the entry of the network.
    norm = cfg.MODEL.RESNETS.NORM
    stem = BasicStem(
        in_channels=input_shape.channels,                 # The channel is 3 for RGB image
        out_channels=cfg.MODEL.RESNETS.STEM_OUT_CHANNELS, # The default stem is 64
        norm=norm,
        deep_stem=deep_stem,
    ...
    #If freeze_at is larger than 1, parameter do not need to be updated
    if freeze_at >= 1:
